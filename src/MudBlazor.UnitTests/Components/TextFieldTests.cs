...
namespace MudBlazor.UnitTests.Components
{
    ...
    public class TextFieldTests : BunitTest
    {
        ...
        {
            var comp = Context.Render<MudTextField<int?>>(Parameter("Value", 17));
            // print the generated html
            comp.Render(Parameter("Value", null));
        ...
        public void WithNoDebounceIntervalValueShouldChangeImmediatelyTest()
        {
            ...
            var comp = Context.Render<MudTextField<string>>(immediate);
            var textField = comp.Instance;
        ...
        public async Task ShouldRespectDebounceIntervalPropertyInTextFieldTest()
        {
            ...
            var comp = Context.Render<MudTextField<string>>(interval);
            var textField = comp.Instance;
            var input = comp.Find("input");
            //Act
            input.Input(new ChangeEventArgs() { Value = "Some Value" });
        ...
        public void LabelShouldShrinkWhenPlaceholderIsSet()
        {
            ...
            var comp = Context.Render<MudTextField<string>>(label);
            comp.Markup.Should().NotContain("shrink");
            //with placeholder label is shrinked
            comp.Render(placeholder);
        ...
        public void TextFieldFluentValidationTest1()
        {
            var validator = new FluentValueValidator<string>(x => x.Cascade(CascadeMode.Stop)
                ...
                .Length(1, 100)
                .CreditCard());
            var comp = Context.Render<MudTextField<string>>(Parameter(nameof(MudTextField<string>.Validation), validator.Validation));
            var textfield = comp.Instance;
        ...
        public void TextField_Should_FireValueChangedOnTextParameterChange()
        {
            ...
            var comp = Context.Render<MudTextField<string>>(EventCallback<string>("ValueChanged", x => changed_value = x));
            comp.Render(Parameter("Text", "A"));
        ...
        public void TextField_Should_FireTextChangedOnValueParameterChange()
        {
            ...
            var comp = Context.Render<MudTextField<string>>(EventCallback<string>("TextChanged", x => changed_text = x));
            comp.Render(Parameter("Value", "A"));
        ...
        {
            var comp = Context.Render<MudTextField<int?>>(Parameter("Required", true));
            var textfield = comp.Instance;
        ...
        public async Task TextField_Should_HaveCorrectMessageWithCustomAttr_Failing()
        {
            ...
            var comp = Context.Render<MudTextField<string>>(Parameter("For", (Expression<Func<string>>)(() => model.Foo)));
        ...
        public async Task TextField_Should_HaveCorrectMessageWithCustomAttr_Override_Failing()
        {
            ...
            var comp = Context.Render<MudTextField<string>>(
                Parameter("For", (Expression<Func<string>>)(() => (model as TestFailingModel2).Foo))
            //Parameter("ForModel", typeof(TestFailingModel2)) // Explicitly set the `For` class
            );
            await comp.InvokeAsync(() => comp.Instance.Validate());
            comp.Instance.Error.Should().BeTrue();
            comp.Instance.ValidationErrors.Should().HaveCount(1);
        ...
        public async Task TextField_Data_Annotation_Resolve_Name_Of_Field()
        {
            ...
            var comp = Context.Render<MudTextField<string>>(Parameter("For", (Expression<Func<string>>)(() => model.Foo1)));
            await comp.InvokeAsync(() => comp.Instance.Validate());
            comp.Instance.Error.Should().BeTrue();
            comp.Instance.ValidationErrors.Should().HaveCount(1);
        ...
        {
            var model = new TestDataAnnotationModel();
            var comp = Context.Render<MudTextField<string>>(Parameter("For", (Expression<Func<string>>)(() => model.Foo2)));
            await comp.InvokeAsync(() => comp.Instance.Validate());
            comp.Instance.Error.Should().BeTrue();
            comp.Instance.ValidationErrors.Should().HaveCount(1);
        ...
        public async Task TextField_Data_Annotation_Compare()
        {
            ...
            var value = "Foo";
            var comp = Context.Render<MudTextField<string>>(
                Parameter("For", (Expression<Func<string>>)(() => model.Foo2)),
        ...
        public async Task TextField_OnlyValidateIfDirty_Is_True_Should_OnlyHaveInputErrorWhenValueChanged()
        {
            var comp = Context.Render<MudTextField<int?>>(
                ...
                Parameter("OnlyValidateIfDirty", true));
            comp.FindAll("div.mud-input-error").Count.Should().Be(0);
        ...
        public async Task TextField_OnlyValidateIfDirty_Is_False_Should_HaveInputErrorWhenFocusChanged()
        {
            var comp = Context.Render<MudTextField<int?>>(
                Parameter("Required", true),
                Parameter("OnlyValidateIfDirty", false));
            comp.FindAll("div.mud-input-error").Count.Should().Be(0);
