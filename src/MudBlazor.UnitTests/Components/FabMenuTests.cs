...
public class FabMenuTests : BunitTest
{
    ...
    public async Task RendersCorrectlyOnHover()
    {
        ...
        await comp.FindAll(".mud-fab-menu-button")[0].MouseEnterAsync(new MouseEventArgs());
        await comp.WaitForAssertionAsync(() => { comp.FindAll(".mud-fab-menu.mud-fab-menu-open").Count.Should().Be(1); });

        await comp.FindAll(".mud-fab-menu-button")[0].MouseLeaveAsync(new MouseEventArgs());
        await comp.FindAll(".mud-fab-menu")[0].MouseEnterAsync(new MouseEventArgs());
        await comp.WaitForAssertionAsync(() => { comp.FindAll(".mud-fab-menu.mud-fab-menu-open").Count.Should().Be(1); });
