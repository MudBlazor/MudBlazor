...
namespace MudBlazor.UnitTests.Components
{
    ...
    public class HiddenTests : BunitTest
    {
        ...
        public void Content_Visible(bool mediaResult, bool invert, bool isHidden)
        {
            ...
            listenerMock.Setup(static x => x.Subscribe(It.IsAny<Action<Breakpoint>>())).ReturnsAsync(new BreakpointServiceSubscribeResult(Guid.NewGuid(), Breakpoint.Md)).Verifiable();
            listenerMock.Setup(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Md)).Returns(mediaResult).Verifiable();
        ...
        public void SizeChanged()
        {
            ...
            var listenerMock = new Mock<IBreakpointService>();
            listenerMock.Setup(static x => x.Subscribe(It.IsAny<Action<Breakpoint>>()))
                .ReturnsAsync(new BreakpointServiceSubscribeResult(Guid.NewGuid(), Breakpoint.Md))
                .Callback<Action<Breakpoint>>(x => callback = x)
                .Verifiable();

            listenerMock.Setup(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Md)).Returns(false).Verifiable();
            listenerMock.Setup(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Xs)).Returns(true).Verifiable();
            listenerMock.Setup(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Xl)).Returns(false).Verifiable();

            Context.Services.AddSingleton(sp => listenerMock.Object);

            var comp = Context.RenderComponent<SimpleMudHiddenTest>(static p =>
            {
                p.Add(static x => x.Breakpoint, Breakpoint.Lg);
                p.Add(static x => x.Invert, false);
            });
        ...
        public void InvertChangedAfterInitializing()
        {
            ...
            comp.SetParametersAndRender(static p => p.Add(static x => x.Invert, true));

            Assert.Throws<ElementNotFoundException>(() => comp.Find("p"));

            listenerMock.Verify(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Md), Times.Exactly(4));
            listenerMock.Verify();
        }
        ...
        public void ReferenceBreakpointChangedAfterInitializing()
        {
            ...
            listenerMock.Setup(static x => x.Subscribe(It.IsAny<Action<Breakpoint>>())).ReturnsAsync(new BreakpointServiceSubscribeResult(Guid.NewGuid(), Breakpoint.Md)).Verifiable();
            listenerMock.Setup(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Md)).Returns(false).Verifiable();
            listenerMock.Setup(static x => x.IsMediaSize(Breakpoint.Xs, Breakpoint.Md)).Returns(true).Verifiable();
            ...
            comp.SetParametersAndRender(static p => p.Add(static x => x.Breakpoint, Breakpoint.Xs));

            Assert.Throws<ElementNotFoundException>(() => comp.Find("p"));

            listenerMock.Verify(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Md), Times.Exactly(2));
            listenerMock.Verify(static x => x.IsMediaSize(Breakpoint.Xs, Breakpoint.Md), Times.Exactly(2));
            listenerMock.Verify();
        }
        ...
        public void WithinMudBreakpointProvider()
        {
            ...
            var comp = Context.RenderComponent<BreakpointProviderWithMudHiddenTest>(static p =>
            {
                p.Add(static x => x.Breakpoint, Breakpoint.Lg);
            });
            ...
            listenerMock.Verify(static x => x.Subscribe(It.IsAny<Action<Breakpoint>>()), Times.Once());
            listenerMock.Verify(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Md), Times.Exactly(8));
            listenerMock.Verify();
        }
        ...
        public void WithinMudBreakpointProvider_UpdateBreakpointValue()
        {
            ...
            listenerMock.Setup(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Md)).Returns(false).Verifiable();
            listenerMock.Setup(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Xs)).Returns(true).Verifiable();
            listenerMock.Setup(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Sm)).Returns(false).Verifiable();
            Context.Services.AddSingleton(sp => listenerMock.Object);
            ...
            listenerMock.Verify(static x => x.Subscribe(It.IsAny<Action<Breakpoint>>()), Times.Once());
            listenerMock.Verify(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Md), Times.Exactly(8));
            listenerMock.Verify(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Xs), Times.Exactly(16));
            listenerMock.Verify(static x => x.IsMediaSize(Breakpoint.Lg, Breakpoint.Sm), Times.Exactly(16));
        ...
        public void TestSemaphore_RenderInParallel()
        {
            ...
            _jsruntimeMock.Setup(static x => x.InvokeAsync<IJSVoidResult>("mudResizeListenerFactory.listenForResize", It.IsAny<object[]>()))
                .ReturnsAsync(Mock.Of<IJSVoidResult>(), TimeSpan.FromMilliseconds(200)).Verifiable();
            _jsruntimeMock.Setup(static x => x.InvokeAsync<IJSVoidResult>("mudResizeListenerFactory.cancelListeners", It.IsAny<object[]>()))
