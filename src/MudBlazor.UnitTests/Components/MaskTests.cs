...
namespace MudBlazor.UnitTests.Components
{
    ...
    public class MaskTests : BunitTest
    {
        ...
        public async Task TextFieldWithMask_SetValueParameterUpdateText(string testName, IMask mask, string initialValue, string setValue)
        {
            ...
            await comp.SetParam(m => m.Value, setValue);
        ...
        public async Task MaskTest_Fundamentals1()
        {
            ...
            await comp.SetParam(x => x.Mask, new PatternMask("(aaa) 000-aa") { Placeholder = '_', CleanDelimiters = true });
            var maskField = comp;
        ...
        public async Task MaskTest_Int()
        {
            ...
            await comp.SetParam(x => x.Mask, new PatternMask("(0)0-0)") { Placeholder = '_', CleanDelimiters = true });
        ...
        {
            var comp = Context.RenderComponent<MudMask>();
            await comp.SetParam(x => x.Mask, new PatternMask("(LL) UU")
            {
                Placeholder = '_',
                CleanDelimiters = true,
                MaskChars = new[]
        ...
        {
            var comp = Context.RenderComponent<MudMask>();
            await comp.SetParam(x => x.Mask, new PatternMask("0000 0000 000") { Placeholder = '_', CleanDelimiters = true });
            var maskField = comp.Instance;
        ...
        public async Task MaskTest_TimeSpan()
        {
            ...
            await comp.SetParam(x => x.Mask, new PatternMask("00:00") { CleanDelimiters = false, });
            var tf = comp.Instance;
            var maskField = comp.FindComponent<MudMask>().Instance;
        ...
        public async Task MaskTest_Readonly()
        {
            ...
            await comp.SetParam(p => p.ReadOnly, false);
            // paste
            await comp.InvokeAsync(() =>
            {
                mask.OnSelect(0, mask.Text.Length);
        ...
        public async Task MetaKeyShortcuts_Should_NotIntroduceExtraCharacters()
        {
            ...
            await comp.SetParam(x => x.Mask, RegexMask.Email());
            var tf = comp.Instance;
            var maskField = comp.FindComponent<MudMask>().Instance;
