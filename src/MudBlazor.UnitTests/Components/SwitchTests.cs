...
namespace MudBlazor.UnitTests.Components
{
    ...
    public class SwitchTest : BunitTest
    {
        ...
        public void SwitchLabelTextSizeTest()
        {
            ...
            comp.FindAll("label.mud-switch")[3].Children[1].ClassList.Should().Contain("mud-switch-label-medium"); //4th switch doesn't have size set, it should be at default values
            comp.FindAll("label.mud-switch")[3].Children[0].ClassList.Should().Contain("mud-switch-span-medium");

            comp.FindAll("label.mud-switch")[4].Children[1].ClassList.Should().Contain("mud-switch-label-small"); //5th switch is a small switch with corresponding label text size
            comp.FindAll("label.mud-switch")[4].Children[0].ClassList.Should().Contain("mud-switch-span-small");

            comp.FindAll("label.mud-switch")[5].Children[1].ClassList.Should().Contain("mud-switch-label-medium"); //6th switch is a medium switch with corresponding label text size
            comp.FindAll("label.mud-switch")[5].Children[0].ClassList.Should().Contain("mud-switch-span-medium");

            comp.FindAll("label.mud-switch")[6].Children[1].ClassList.Should().Contain("mud-switch-label-large"); //7th switch is a large switch with corresponding label text size
            comp.FindAll("label.mud-switch")[6].Children[0].ClassList.Should().Contain("mud-switch-span-large");

            comp.FindAll("label.mud-switch")[7].Children[1].ClassList.Should().Contain("mud-switch-label-small"); //8th switch is a small switch that changes to large when unchecked
            comp.FindAll("label.mud-switch")[7].Children[0].ClassList.Should().Contain("mud-switch-span-small");

            // 8th switch Size is tied to the Label_Switch2 bool, if it's false, it should become large
            comp.FindAll("input")[7].Change(false);
            comp.FindAll("label.mud-switch")[7].Children[1].ClassList.Should().Contain("mud-switch-label-large");
            comp.FindAll("label.mud-switch")[7].Children[0].ClassList.Should().Contain("mud-switch-span-large");
        }
