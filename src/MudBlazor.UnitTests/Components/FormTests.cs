...
namespace MudBlazor.UnitTests.Components
{
    ...
    public class FormTests : BunitTest
    {
        ...
        public async Task Form_Should_Validate_ColorPicker_When_ColorSelectedViaInputs()
        {
            ...
            await colorPickerComp.SetParam(x => x.Validation, new Func<MudColor, string>(color => color != null && color.Value == forbiddenColor.Value ? $"{forbiddenColor.Value} is not allowed" : null));
            ...
            await comp.SetParam(x => x.ColorValue, forbiddenColor);
            form.IsValid.Should().Be(false);
            form.Errors.Length.Should().Be(1);
        ...
        public async Task FormWithDatePickerTest()
        {
            ...
            datepicker.GetState(x => x.ErrorText).Should().BeNullOrEmpty();
            // clear selection
            await comp.SetParam(x => x.Date, null);
            form.IsValid.Should().Be(false);
            form.Errors.Length.Should().Be(1);
            form.Errors[0].Should().Be("Required");
        ...
        public async Task Form_Should_ValidateDatePickerTest()
        {
            ...
            var dateComp = comp.FindComponent<MudDatePicker>();
            var datepicker = comp.FindComponent<MudDatePicker>().Instance;
            await dateComp.SetParam(x => x.Validation, new Func<DateTime?, string>(date => date != null && date.Value.Year >= 2000 ? null : "Year must be >= 2000"));
            dateComp.Find("input").Change(new DateTime(2001, 01, 31).ToShortDateString());
            form.IsValid.Should().Be(true);
            form.Errors.Length.Should().Be(0);
            datepicker.Error.Should().BeFalse();
            datepicker.GetState(x => x.ErrorText).Should().BeNullOrEmpty();
            // set invalid date:
            await comp.SetParam(x => x.Date, (DateTime?)new DateTime(1999, 1, 1));
            form.IsValid.Should().Be(false);
            form.Errors.Length.Should().Be(1);
        ...
        public async Task Form_Should_Validate_DateRangePicker_When_DateRangeSelectedViaInputs()
        {
            ...
            dateRangePicker.GetState(x => x.ErrorText).Should().BeNullOrEmpty();
            // clear selection
            await comp.SetParam(x => x.DateRange, null);
            form.IsValid.Should().Be(false);
            form.Errors.Length.Should().Be(1);
            form.Errors[0].Should().Be("Required");
        ...
        public async Task FormWithTimePickerTest()
        {
            ...
            timePicker.GetState(x => x.ErrorText).Should().BeNullOrEmpty();
            // clear selection
            await comp.SetParam(x => x.Time, null);
            form.IsValid.Should().Be(false);
            form.Errors.Length.Should().Be(1);
            form.Errors[0].Should().Be("Required");
        ...
        public async Task Form_Should_ValidateTimePickerTest()
        {
            ...
            var timePicker = comp.FindComponent<MudTimePicker>().Instance;
            await timeComp.SetParam(x => x.Validation, new Func<TimeSpan?, string>(time => time != null && time.Value.Minutes == 0 ? null : "Only full hours allowed"));
            ...
            timePicker.GetState(x => x.ErrorText).Should().BeNullOrEmpty();
            // set invalid date:
            await comp.SetParam(x => x.Time, (TimeSpan?)new TimeSpan(0, 17, 05, 00)); // "17:05"
            form.IsValid.Should().Be(false);
            form.Errors.Length.Should().Be(1);
        ...
        public async Task FormSpacingClass()
        {
            ...
            for (var i = 0; i <= 20; i++)
            {
                await comp.SetParam(x => x.Spacing, i);
