...
namespace MudBlazor.UnitTests.Components
{
    ...
    public class FormTests : BunitTest
    {
        ...
        [Test]
        public async Task FormIsValid_WithRequiredDatePicker_ShouldRemainFalseUntilDateIsSet()
        {
            var comp = Context.Render<FormIsValidWithDatePickerTest>();
            var form = comp.FindComponent<MudForm>().Instance;
            var textField = comp.FindComponent<MudTextField<string>>();
            var datePicker = comp.FindComponent<MudDatePicker>().Instance;
            var isValidSwitch = comp.FindComponent<MudSwitch<bool>>().Instance;

            form.IsValid.Should().BeFalse();
            isValidSwitch.ReadValue.Should().BeFalse();
            datePicker.Touched.Should().BeFalse();

            await textField.Find("input").InputAsync("Filled required text");
