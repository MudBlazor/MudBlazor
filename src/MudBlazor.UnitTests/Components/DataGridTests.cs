    ...
    public class DataGridTests : BunitTest
    {
        ...
        public void DataGrid_SetParameters_ServerData_Items_Throw()
        {
            ...
            var exception = Assert.Throws<InvalidOperationException>(() =>
                Context.Render<MudDataGrid<TestModel1>>(
        ...
        public async Task DataGridEventCallbacksTest()
        {
            ...
            var parameters = new[]
            {
                Parameter(nameof(dataGrid.Instance.MultiSelection), true),
                Parameter(nameof(dataGrid.Instance.ReadOnly), false),
                Parameter(nameof(dataGrid.Instance.EditMode), DataGridEditMode.Cell),
                Parameter(nameof(dataGrid.Instance.EditTrigger), DataGridEditTrigger.OnRowClick)
            };
            dataGrid.Render(parameters);
        ...
        public void FilterDefinitionStringTest()
        {
            ...
            dataGrid.Render(parameters => parameters.Add(parameter => parameter.FilterCaseSensitivity, DataGridFilterCaseSensitivity.CaseInsensitive));
            func = filterDefinition.GenerateFilterFunction(new FilterOptions
            {
                FilterCaseSensitivity = dataGrid.Instance.FilterCaseSensitivity
            });
            ...
            dataGrid.Render(parameters => parameters.Add(parameter => parameter.FilterCaseSensitivity, DataGridFilterCaseSensitivity.Default));
            func = filterDefinition.GenerateFilterFunction(new FilterOptions
            {
                FilterCaseSensitivity = dataGrid.Instance.FilterCaseSensitivity
            });
        ...
        public void DataGridShowFilterIconTest()
        {
            ...
            parameters.Clear();
            parameters.Add(Parameter(nameof(dataGrid.Instance.Filterable), true));
            dataGrid.Render(parameters.ToArray());
            dataGrid.Instance.DropContainerHasChanged();
            dataGrid.FindAll(".filter-button").Should().NotBeEmpty();
            parameters.Clear();
            parameters.Add(Parameter(nameof(dataGrid.Instance.ShowFilterIcons), false));
            dataGrid.Render(parameters.ToArray());
            dataGrid.Instance.DropContainerHasChanged();
            dataGrid.FindAll(".filter-button").Should().BeEmpty();
        }
        ...
        public async Task DataGridUniqueRowKey()
        {
            ...
            comp.Render(parameters => parameters.Add(p => p.Group, true));
        ...
        public async Task DataGridGetHierarchyGroupIconTest()
        {
            ...
            comp.Render(parameters => parameters.Add(p => p.RightToLeft, true));
            // When collapsed + RTL
        ...
        public void DataGrid_HierarchyExpandSingleRowTest()
        {
            ...
            comp.Render(p => p.Add(p => p.ExpandSingleRow, true));
        ...
        public void DataGridFilterIconsTest()
        {
            ...
            comp.Render(parameters => parameters.Add(p => p.FilterMode, DataGridFilterMode.ColumnFilterMenu));

            mudIconButton = FirstFilterButton();
            mudIconButton.Icon.Should().Be(Icons.Material.Filled.Battery0Bar);

            // Check filter buttons when filter applied
            comp.Render(parameters => parameters.Add(p => p.FilterMode, DataGridFilterMode.Simple));
            ...
            comp.Render(parameters => parameters.Add(p => p.FilterMode, DataGridFilterMode.ColumnFilterRow));
