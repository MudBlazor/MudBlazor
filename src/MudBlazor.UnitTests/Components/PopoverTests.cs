    ...
    public class PopoverTests : BunitTest
    {
        ...
        public async Task MudPopoverHandler_InitizeAndDetach()
        {
            ...
            x.InvokeAsync<object>(
                "mudPopover.connect",
                It.Is<object[]>(x => x.Length == 1 && (Guid)x[0] == handlerId))).ReturnsAsync(Mock.Of<object>).Verifiable();
        ...
        public async Task MudPopoverService_Constructor_NullOption()
        {
           ...
           x.InvokeAsync<object>(
               ...
               It.Is<object[]>(x => x.Length == 2 && (string)x[0] == "mudblazor-main-content" && (int)x[1] == 0))).ReturnsAsync(Mock.Of<object>).Verifiable();
        ...
        public async Task MudPopoverService_Constructor_OptionWithCustomClass()
        {
           ...
           x.InvokeAsync<object>(
               "mudPopover.initilize",
               It.Is<object[]>(x => x.Length == 2 && (string)x[0] == "my-custom-class" && (int)x[1] == 12))).ReturnsAsync(Mock.Of<object>).Verifiable();
        ...
        public async Task MudPopoverService_CallInitializeOnlyOnce()
        {
           ...
           x.InvokeAsync<object>(
               "mudPopover.initilize",
               It.Is<object[]>(x => x.Length == 2 && (string)x[0] == "mudblazor-main-content" && (int)x[1] == 0))).ReturnsAsync(Mock.Of<object>(), TimeSpan.FromMilliseconds(300)).Verifiable();
        ...
        public async Task MudPopoverService_DisposeAsync()
        {
           ...
           x.InvokeAsync<object>(
            ...
            x.InvokeAsync<object>(
            "mudPopover.dispose",
            It.Is<object[]>(x => x.Length == 0))).ReturnsAsync(Mock.Of<object>).Verifiable();
