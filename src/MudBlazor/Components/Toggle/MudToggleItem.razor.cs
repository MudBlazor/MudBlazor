...
#nullable enable
    public partial class MudToggleItem<T> : MudComponentBase
    {
        ...
        protected string Classes => new CssBuilder("mud-toggle-item")
            ...
            //.AddClass("border-r border-b", Parent?.ShowOutline == true)
            //.AddClass("border-l", Parent?.ShowOutline == true && (Parent?.Vertical == true || Parent?.IsFirstItem(this) == true || Parent?.RightToLeft == true))
            //.AddClass("border-t", Parent?.ShowOutline == true && (Parent?.Vertical == false || Parent?.IsFirstItem(this) == true))
