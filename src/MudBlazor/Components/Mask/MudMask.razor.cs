...
public partial class MudMask : MudBaseInput<string>
{
    ...
    protected string Classname =>
        new CssBuilder("mud-input")
           ...
           .AddClass("mud-input-underline", () => DisableUnderLine == false && Variant != Variant.Outlined)
           .AddClass("mud-shrink",
                     () => !string.IsNullOrEmpty(Text) || Adornment == Adornment.Start ||
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            await _jsEvent.Connect(_elementId,
                                   ...
                                   {
                                       //EnableLogging = true,
                                       TargetClass = "mud-input-slot", TagName = "INPUT"
                                   });
        ...
        {
            SetCaretPosition(_caret, _selection, false);
        }
    ...
    private async Task Update()
    {
        ...
        try
        {
            await base.SetTextAsync(text, false);
            if (Clearable)
            {
