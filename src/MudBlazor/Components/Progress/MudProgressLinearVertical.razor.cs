...
namespace MudBlazor
{
    public partial class MudProgressLinearVertical : MudComponentBase
    {
        protected string DivClassname =>
            new CssBuilder("mud-progress-linear-vertical")
                .AddClass($"mud-progress-linear-vertical-color-{Color.ToDescriptionString()}", !Buffer)
                .AddClass("mud-flip-x-rtl")
                .AddClass(Class)
                .Build();

        protected string LinearClassname =>
            new CssBuilder("mud-progress-linear-vertical-bar")
                .AddClass($"mud-{Color.ToDescriptionString()}")
                .AddClass($"mud-progress-indeterminate", Indeterminate)
                .AddClass($"mud-progress-linear-vertical-bar-1-determinate", !Indeterminate)
                .Build();

        protected string BufferClassname =>
            new CssBuilder("mud-progress-linear-vertical-dashed")
                .AddClass($"mud-progress-linear-vertical-dashed-color-{Color.ToDescriptionString()}")
                .Build();
