...
namespace MudBlazor
{
    public partial class MudTreeViewItemEx<T> : MudComponentBase
    {
        ...
        new CssBuilder("mud-treeview-item-content")
          .AddClass("cursor-pointer",
                (MudTreeRoot?.IsSelectable == true && (!Disabled || MudTreeRoot?.CanSelectDisabledItem == true)) ||
                (MudTreeRoot?.ExpandOnClick == true && HasChild))
        ...
        [CascadingParameter] MudTreeViewItemEx<T> Parent { get; set; }
        ...
        [Parameter]
        [Category(CategoryTypes.TreeView.Behavior)]
        public RenderFragment<MudTreeViewItemEx<T>> BodyContent { get; set; }
...
#pragma warning restore CS0618
        ...
        internal async Task SelectItem(bool value, MudTreeViewItemEx<T> source = null)
        {
            if (value == _isChecked)
                return;
        ...
        private void AddChild(MudTreeViewItemEx<T> item) => _childItems.Add(item);

        internal IEnumerable<MudTreeViewItemEx<T>> GetSelectedItems()
        {
            if (_isChecked)
                yield return this;
