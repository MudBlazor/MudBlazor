...
#nullable enable
    public class ApiLinkService : IApiLinkService
    {
        ...
        private double GetSearchMatchRatio(string search, string keyword)
        {
            ...
            var searchMatchRatio = GetSearchFirstMatchRatio(search, keyword);
            if (searchMatchRatio > 0)
            {
                return searchMatchRatio;
            }

            var tokenTypoMatchRatio = GetSingleTokenTypoMatchRatio(search, keyword);
            if (tokenTypoMatchRatio > 0)
            {
                return tokenTypoMatchRatio;
            }

            var ratio = Fuzz.Ratio(keyword, search);
            var partialRatio = Fuzz.PartialRatio(keyword, search);

            return Math.Max(ratio, Math.Min(partialRatio, 80));
        }

        private static double GetSearchFirstMatchRatio(string search, string keyword)
        {
            if (keyword.StartsWith(search, StringComparison.Ordinal))
            {
                return 100 - GetLengthPenalty(search, keyword, 12);
            }
            ...
            if (searchTokens.Length > 1 && IsOrderedTokenPrefixMatch(searchTokens, keywordTokens))
            {
                return 98 - GetLengthPenalty(search, keyword, 10);
            }

            var index = keyword.IndexOf(search, StringComparison.Ordinal);
            if (index >= 0)
            {
                return 94 - GetLengthPenalty(search, keyword, 8) - Math.Min(index * 0.1, 4);
            }

            if (searchTokens.Length == 1 && keywordTokens.Any(token => token.StartsWith(search, StringComparison.Ordinal)))
            {
                return 92 - GetLengthPenalty(search, keyword, 8);
            }

            return 0;
        }

        private static double GetSingleTokenTypoMatchRatio(string search, string keyword)
        {
            if (search.Contains(' ', StringComparison.Ordinal) || search.Length < 4)
            {
                return 0;
            }
            ...
            foreach (var keywordToken in keywordTokens)
            {
                var normalizedKeywordToken = keywordToken.Trim(',', '.', ':', ';', '!', '?', '(', ')');
                if (normalizedKeywordToken.Length < 4 || Math.Abs(normalizedKeywordToken.Length - search.Length) > 2)
                {
                    continue;
                }

                var editDistance = GetEditDistance(search, normalizedKeywordToken);
                if (editDistance > 2)
                {
                    continue;
                }

                bestRatio = Math.Max(bestRatio, 91 - (editDistance * 4));
            }

            return bestRatio;
        }

        private static double GetLengthPenalty(string search, string keyword, double maxPenalty)
        {
            return Math.Min(Math.Max(keyword.Length - search.Length, 0) * 0.25, maxPenalty);
        }

        private static int GetEditDistance(string search, string keyword)
        {
            var previousRow = new int[keyword.Length + 1];
            var currentRow = new int[keyword.Length + 1];
            ...
            {
                currentRow[0] = searchIndex + 1;

                for (var keywordIndex = 0; keywordIndex < keyword.Length; keywordIndex++)
                {
                    var substitutionCost = search[searchIndex] == keyword[keywordIndex] ? 0 : 1;
                    currentRow[keywordIndex + 1] = Math.Min(
                        Math.Min(currentRow[keywordIndex] + 1, previousRow[keywordIndex + 1] + 1),
                        previousRow[keywordIndex] + substitutionCost);
                }

                (previousRow, currentRow) = (currentRow, previousRow);
            }

            return previousRow[keyword.Length];
        }

        private static bool IsOrderedTokenPrefixMatch(IReadOnlyList<string> searchTokens, IReadOnlyList<string> keywordTokens)
        {
            var keywordTokenIndex = 0;

            foreach (var searchToken in searchTokens)
            {
                var foundToken = false;

                while (keywordTokenIndex < keywordTokens.Count)
                {
                    if (keywordTokens[keywordTokenIndex].StartsWith(searchToken, StringComparison.Ordinal))
                    {
                        foundToken = true;
                        keywordTokenIndex++;
                        break;
